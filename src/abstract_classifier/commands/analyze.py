...
DESCRIPTION = (
    "Run the governed methodology, theme, and optional client-reporting bundle over a classified artifact."
)
...
def handle_command(args: argparse.Namespace) -> int:
    ...
    if artifacts.reporting_artifacts:
        print(f"Client results: {artifacts.reporting_artifacts['client_results']}")
        print(f"Client report: {artifacts.reporting_artifacts['report']}")
    return 0
...
def register(subparsers) -> None:
    ...
    parser.add_argument(
        "--reporting-config",
        default="configs/client_reporting.toml",
        help="Path to the Phase 5 client reporting config TOML.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enable-reporting",
        action="store_true",
        help="Generate Phase 5 client-ready reporting artifacts from this analysis run.",
    )
