...
def build_text_variant_frame(
    ...
    *,
    ...
    merged = attach_text_metadata(dataset_rows, text_metadata=text_metadata)
    merged["keywords_applied"] = False

    if variant_name == TEXT_VARIANT_ABSTRACT_ONLY:
        merged["text_input"] = merged["abstract"].fillna("").map(str)
        return merged
    if variant_name == TEXT_VARIANT_TITLE_ABSTRACT_PLUS_KEYWORDS:
        merged["keywords_applied"] = merged["keywords_available"]
        merged["text_input"] = merged.apply(_compose_title_keyword_enriched_text, axis=1)
        return merged
...
def summarize_keyword_coverage(
    ...
    *,
    ...
    summary_frame = variant_rows.copy()
    if "source_dataset" not in summary_frame.columns:
        summary_frame["source_dataset"] = "unknown"
    if "split" not in summary_frame.columns:
        summary_frame["split"] = "unspecified"

    summary_rows = (
        summary_frame.groupby(["source_dataset", "split"], dropna=False)
        .agg(
...
def attach_text_metadata(
    ...
    *,
    ...
    inline_metadata = _extract_inline_text_metadata(dataset_rows)
    metadata = inline_metadata if inline_metadata is not None else text_metadata
    if metadata is None:
        raise ValueError(
            "Text metadata is required when the dataset rows do not already "
            "carry inline keyword columns."
        )

    base_rows = dataset_rows.drop(
        columns=[
            column
            for column in ("author_keywords", "index_keywords", "keywords_available")
            if column in dataset_rows.columns
        ],
        errors="ignore",
    )
    merged = base_rows.merge(
        metadata.loc[:, list(_INLINE_TEXT_METADATA_COLUMNS)],
        on="record_id",
        how="left",
        validate="one_to_one",
    )
...
def _extract_inline_text_metadata(dataset_rows: pd.DataFrame) -> pd.DataFrame | None:
    if "record_id" not in dataset_rows.columns:
        return None
    if not any(
        column in dataset_rows.columns
        for column in ("author_keywords", "index_keywords", "keywords_available")
    ):
        return None

    metadata = dataset_rows.loc[:, ["record_id"]].copy()
    author_keywords = (
        dataset_rows["author_keywords"]
        if "author_keywords" in dataset_rows.columns
        else pd.Series([""] * len(dataset_rows), index=dataset_rows.index)
    )
    index_keywords = (
        dataset_rows["index_keywords"]
        if "index_keywords" in dataset_rows.columns
        else pd.Series([""] * len(dataset_rows), index=dataset_rows.index)
    )
    metadata["author_keywords"] = author_keywords.fillna("").map(str).str.strip()
    metadata["index_keywords"] = index_keywords.fillna("").map(str).str.strip()

    if "keywords_available" in dataset_rows.columns:
        metadata["keywords_available"] = (
            dataset_rows["keywords_available"].fillna(False).astype(bool)
        )
    else:
        metadata["keywords_available"] = (
            metadata["author_keywords"].ne("") | metadata["index_keywords"].ne("")
        )
    return metadata
...
def _compose_keyword_enriched_text(row: pd.Series) -> str:
    ...
    return _compose_keyword_sections(
        abstract=abstract,
...
def _compose_title_keyword_enriched_text(row: pd.Series) -> str:
    title = stringify_label(row.get("title")).strip()
    abstract = stringify_label(row.get("abstract")).strip()
