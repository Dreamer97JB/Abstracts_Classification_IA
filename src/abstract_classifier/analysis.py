...
from .client_reporting import (
    DEFAULT_CLIENT_REPORTING_CONFIG,
...
class AnalysisRunArtifacts:
    ...
    reporting_artifacts: dict[str, Path]
...
def run_analysis_bundle(
    *,
    ...
    reporting_config_path: str | Path = DEFAULT_CLIENT_REPORTING_CONFIG,
    ...
    reporting_config = load_client_reporting_config(
    ...
    if enable_reporting:
        reporting_bundle = build_client_reporting_bundle(
        ...
        reporting_artifacts = {
            "client_results": reporting_bundle.client_results_path,
            "label_theme_correlations": reporting_bundle.label_theme_correlations_path,
            "label_keyword_correlations": reporting_bundle.label_keyword_correlations_path,
            "author_summary": reporting_bundle.author_summary_path,
            "reference_summary": reporting_bundle.reference_summary_path,
            "summary": reporting_bundle.summary_path,
            "report": reporting_bundle.report_path,
        }
    ...
    manifest = {
        ...
        "reporting": {
            "enabled": enable_reporting,
            "config_path": _relative_path(reporting_config.config_path, project_root),
            "artifacts": {
                key: _relative_path(path, project_root)
                for key, path in reporting_artifacts.items()
            },
        },
